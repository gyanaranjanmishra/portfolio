...
import { publications } from "@/lib/config";
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <p className="text-gray-400 text-sm">{publications.length} papers — click &quot;Summarize&quot; for an AI-generated summary</p>
        {publications.map((pub, i) => (
          <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-2">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <h2 className="font-semibold text-white leading-snug">{pub.title}</h2>
                <p className="text-gray-400 text-sm mt-1">{pub.authors}</p>
                <p className="text-blue-400 text-sm">{pub.journal} · {pub.year}</p>
              </div>
